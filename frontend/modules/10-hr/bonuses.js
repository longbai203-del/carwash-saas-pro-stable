...
/** @type {{records: PenaltyRecord[], filteredRecords: PenaltyRecord[], filters: {type: string, status: string, department: string, employee: string, dateFrom: string, dateTo: string}, stats: {totalAmount: number, deductedAmount: number, pendingAmount: number, totalRecords: number}, page: number, pageSize: number}} 状态 */
const state = {
    records: [],
    filteredRecords: [],
    filters: {
...
function getMockPenalties() {
    ...
    const types = ['late', 'violation', 'quality', 'attendance', 'late', 'violation'];
    const amounts = [50, 200, 100, 80, 50, 150];
    const reasons = ['上班迟到30分钟', '违反操作规范', '服务投诉', '无故缺勤', '迟到15分钟', '损坏设备'];
    const statuses = ['pending', 'deducted', 'pending', 'deducted', 'pending', 'deducted'];
...
function updateStats() {
    ...
    document.getElementById('statDeductedAmount')?.textContent = '¥' + formatCurrency(stats.deductedAmount);
