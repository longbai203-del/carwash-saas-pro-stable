 ...
 * @module vehicle-monitor
 * @description 车辆监控模块 - 车辆进出记录、停留时间追踪、实时计数、车牌智能识别
 * 
 * @example
 * import { init } from './vehicle-monitor.js';
 * await init();
 * 
 ...
 * @description 初始化车辆监控模块
 */
export async function init() {
    if (typeof VehicleMonitorModule !== 'undefined' && !VehicleMonitorModule.initialized) {
        VehicleMonitorModule.cacheDom();
        VehicleMonitorModule.bindEvents();
        VehicleMonitorModule.loadData();
 ...
 */
export function quickEntry(plate) {
    if (typeof VehicleMonitorModule !== 'undefined') {
        VehicleMonitorModule.quickEntry(plate);
    }
}
 ...
 */
export function quickExit(plate) {
    if (typeof VehicleMonitorModule !== 'undefined') {
        VehicleMonitorModule.quickExit(plate);
    }
}

/**
 * @public
 * @description 销毁模块（清理资源）
 */
