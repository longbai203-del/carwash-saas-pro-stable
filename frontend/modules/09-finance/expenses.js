...
/** @type {{records: ExpenseRecord[], filteredRecords: ExpenseRecord[], filters: {category: string, method: string, dateFrom: string, dateTo: string}, stats: {total: number, count: number, byCategory: Object}, page: number, pageSize: number, editingId: string|null}} 状态 */
const state = {
    records: [],
    filteredRecords: [],
    filters: {
...
/**
 * @private
 * @returns {ExpenseRecord[]} 模拟支出数据
 */
function getMockExpenses() {
    const categories = ['采购', '人力', '租金', '营销', '设备', '其他'];
    const descriptions = ['采购洗车液', '员工工资', '场地租金', '广告投放', '设备维修', '水电费', '办公用品', '差旅费'];
    const methods = ['微信', '支付宝', '现金', '银行卡', '银行转账'];
    const suppliers = ['供应商A', '供应商B', '员工', '物业', '广告公司', '维修公司'];
    ...
    tbody.innerHTML = pageData.map(r => {
        ...
        return `
            <tr style="border-bottom:1px solid #F3F4F6;transition:background 0.2s;"
                ...
                <td style="padding:10px 16px;font-size:13px;">${r.supplier}</td>
...
function renderPagination() {
    ...
    html += `
        <button onclick="window.ExpensesModule.goToPage(${state.page - 1})" ${state.page <= 1 ? 'disabled' : ''} 
    ...
    if (startPage > 1) {
        html += `<button onclick="window.ExpensesModule.goToPage(1)" style="padding:4px 10px;border:1px solid #D1D5DB;border-radius:4px;background:white;cursor:pointer;">1</button>`;
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        html += `
            <button onclick="window.ExpensesModule.goToPage(${i})" 
    ...
    if (endPage < totalPages) {
        ...
        html += `<button onclick="window.ExpensesModule.goToPage(${totalPages})" style="padding:4px 10px;border:1px solid #D1D5DB;border-radius:4px;background:white;cursor:pointer;">${totalPages}</button>`;
    }
    
    html += `
        <button onclick="window.ExpensesModule.goToPage(${state.page + 1})" ${state.page >= totalPages ? 'disabled' : ''}
...
function showCreateModal() {
    const category = prompt('分类 (采购/人力/租金/营销/设备/其他)：', '采购');
    if (!category) return;
    ...
    const method = prompt('支付方式 (微信/支付宝/现金/银行卡/银行转账)：', '微信') || '微信';
    const supplier = prompt('供应商：', '供应商A') || '供应商A';
